...
const KEYWORD_DOCS: Partial<Record<TokenKind, string>> = {
  ...
  [TokenKind.APAVAADA]:   '**apavaada** (अपवाद) — Per-step exception path. `apavaada → step-name` routes to a named error-handler step if this step fails. Each step can have its own failure path.',
  [TokenKind.SAMAPTI]:    '**samapti** (समाप्ति) — SLA timeout path. `samapti → step-name` routes to an escalation step when the step\'s `samaya` (time limit) is exceeded. Requires `samaya` to be set on the same step.',
